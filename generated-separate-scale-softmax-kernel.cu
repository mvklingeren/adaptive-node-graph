...
// ======================================================
      ...
      __global__ void scale_forward(Tensor<float> output, Tensor<float> input) {
        ...
        for (int i = idx; i < size; i += gridDim.x * blockDim.x) {
            output.data[i] = input.data[i] * 0.125f;
        }
      }
...
extern "C" void executeGraph(
  ...
  const int intermediate_0_shape[] = {2, 8, 128, 128};

  // --- Tensor Struct Instantiation ---
  float* intermediate_0_data = (float*)allocator.allocate(1048576);
  if (!intermediate_0_data) { fprintf(stderr, "Failed to allocate memory for intermediate_0\n"); return; }
  Tensor<float> intermediate_0_tensor = {intermediate_0_data, intermediate_0_shape, 4};
  Tensor<float> input = {(float*)input_data, input_shape, input_dims};
  Tensor<float> output = {output_data, output_shape, output_dims};

  // --- Kernel Launch Sequence ---
  scale_forward<<<dim3(1024, 1, 1), dim3(256, 1, 1), 0>>>(intermediate_0_tensor, input);
  CUDA_CHECK(cudaGetLastError());
  softmax_forward<<<dim3(128, 8, 2), dim3(128, 1, 1), 512>>>(output, intermediate_0_tensor);
  CUDA_CHECK(cudaGetLastError());
