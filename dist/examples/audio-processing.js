...
const envelope = createProcessor((time) => {
    ...
    if (time < 0.1)
        return time / 0.1; // Attack
    if (time < 0.2)
        return 1; // Decay to sustain
    if (time < 0.8)
        return 0.7; // Sustain
...
const gain = createProcessor(([samples, gainValue]) => samples.map(s => s * gainValue), 'gain');
graph.addNode(osc);
...
// Apply envelope
const envelopeValues = await Promise.all(Array.from({ length: 44100 }, (_, i) => envelope.process(i / 44100)));
