...
export class Graph {
    ...
    async executeParallel(input) {
        ...
        while (executed.size < this.nodes.size) {
            const readyNodes = Array.from(this.nodes.values()).filter(node => !executed.has(node.id) && canExecute(node.id));
...
// ============================================================================
export const createDelayNode = (ms) => new AdaptiveNode(async (input) => {
    await new Promise(resolve => setTimeout(resolve, ms));
    return input;
}).setLabel(`delay(${ms}ms)`);
export const createThrottleNode = (ms) => {
    let lastEmit = 0;
...
export const createDebounceNode = (ms) => {
    let timeoutId = null;
    let resolver = null;
...
// ============================================================================
export const createErrorLoggerNode = () => new AdaptiveNode((error) => {
    console.error(`[${new Date(error.timestamp).toISOString()}] Error in node ${error.nodeId}:`, error.error.message, '\nInput:', error.input);
    return error;
}).setLabel('errorLogger');
export const createErrorRecoveryNode = (defaultValue) => new AdaptiveNode((error) => {
    console.warn(`Recovering from error with default value:`, defaultValue);
    return defaultValue;
}).setLabel('errorRecovery');
export const createRetryNode = (maxRetries = 3, delayMs = 1000) => {
...
// Testing Utilities
// ============================================================================
export class TestNode extends AdaptiveNode {
    receivedInputs = [];
    processedOutputs = [];
    errors = [];
...
export async function testGraph(graph, input, expectedOutput, startNodeId) {
...
// Sub-graph Support
// ============================================================================
export class SubGraphNode extends AdaptiveNode {
    subGraph;
    inputNodeId;
    outputNodeId;
...
export const createAddNode = () => new AdaptiveNode((inputs) => inputs.reduce((a, b) => a + b, 0)).setLabel('+');
export const createMultiplyNode = () => new AdaptiveNode((inputs) => inputs.reduce((a, b) => a * b, 1)).setLabel('*');
export const createSubtractNode = () => new AdaptiveNode(([a, b]) => a - b).setLabel('-');
export const createDivideNode = () => new AdaptiveNode(([a, b]) => b !== 0 ? a / b : 0).setLabel('/');
// Float32Array Operators (original)
export const createFloat32MultiplyNode = () => new AdaptiveNode((input) => {
...
export const createConditionalNode = () => new AdaptiveNode(([condition, ifTrue, ifFalse]) => condition ? ifTrue : ifFalse).setLabel('?');
export const createAndNode = () => new AdaptiveNode((inputs) => inputs.every(Boolean)).setLabel('&&');
export const createOrNode = () => new AdaptiveNode((inputs) => inputs.some(Boolean)).setLabel('||');
export const createNotNode = () => new AdaptiveNode((input) => !input).setLabel('!');
// Data Flow Operators (enhanced)
export const createGateNode = () => new AdaptiveNode(([pass, data]) => pass ? data : null).setLabel('gate');
export const createMergeNode = () => new AdaptiveNode((inputs) => inputs.filter(x => x !== null && x !== undefined)).setLabel('merge');
export const createSplitNode = (count = 2) => {
    const node = new AdaptiveNode((input) => Array(count).fill(input)).setLabel('split');
...
export const createRouterNode = () => new AdaptiveNode((input) => ({ route: 'default', data: input }))
    ...
    if ('sampleRate' in obj)
        return { route: 'audio', data: obj };
    if ('buffer' in obj)
        return { route: 'binary', data: obj };
    return { route: 'object', data: obj };
})
    .setLabel('router');
// Load Balancer (enhanced with health checks)
export function createLoadBalancerNode(nodes, strategy = 'round-robin') {
    let index = 0;
...
export function createParallelNode(nodes) {
    return new AdaptiveNode(async (input) => {
        return Promise.all(nodes.map(node => node.process(input)));
    }).setLabel('parallel');
}
// Cache Node (fixed implementation)
export function createCacheNode(processor, ttl = 1000, maxSize = 100) {
    const cache = new Map();
    return new AdaptiveNode(async (input) => {
...
// Utility Functions
// ============================================================================
export function createProcessor(fn, label) {
    const node = new AdaptiveNode(fn);
    if (label)
        node.setLabel(label);
    return node;
}
export function chain(first, second) {
    return new AdaptiveNode(async (input) => {
