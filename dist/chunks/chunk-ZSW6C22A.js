...
var AdaptiveNode = class {
  ...
  handleError(error, input, graph, hooks) {
    ...
    if (this.errorCount >= this.circuitBreakerThreshold) {
      ...
      const logger = hooks?.logger || (0, import_pino.default)();
      logger.error(
...
var createErrorLoggerNode = (logger) => new AdaptiveNode((error) => {
  const log = logger || (0, import_pino.default)();
  log.error(
    {
...
var createRouterNode = () => new AdaptiveNode((input) => ({
  ...
  (input) => input instanceof AudioBuffer,
  (audio) => ({ route: "audio", data: audio })
).register(
  (input) => input instanceof ArrayBuffer,
...
var LoadBalancerNode = class extends AdaptiveNode {
  constructor(nodes, strategy = "round-robin", healthCheckInterval = 3e4, logger = (0, import_pino.default)()) {
    super(async () => null);
    this.nodes = nodes;
  ...
  async process(input, graph, hooks) {
    ...
    let result = null;
    const originalEmitState = selectedNode.isEmitting;
    try {
      selectedNode.setEmitting(false);
      result = await selectedNode.process(input, graph, hooks);
      selectedNode.setEmitting(originalEmitState);
      if (result === null) {
        const originalError = selectedNode.getLastError();
        throw originalError || new Error(`Node ${selectedNode.id} returned null without a specific error.`);
      }
      if (this.outlets[0]) {
        await this.outlets[0].send(result, graph, hooks);
      }
