...
export class CudaGraphCompiler {
  ...
  async compile(
    ...
    const { kernelCode, workspaceSize, filename } = this.generateKernelCode(
      graph,
      paramNames,
      paramNameMapping
    );
  ...
  generateKernelCode(
    ...
    for (const node of executionOrder) {
      ...
      const paramArgs = Array.from(node.parameters.keys()).map(
        (localParamName) => {
          const nodeParamKey = `${node.id}:${localParamName}`;
          const globalParamName = paramNameMapping.get(nodeParamKey);
          if (!globalParamName) {
            throw new Error(
              `Compiler Error: Could not find global parameter name for ${node.name}:${localParamName}`
            );
          }
      ...
      const kernelArgs = [
        ...Array.from(node.outputs.keys()).map((name) =>
          outputTensors.get(name)
        ),
      ...
      executionCalls.push(
        `  ${node.functionName}<<<${gridDim}, ${blockDim}, ${sharedMemSize}>>>(${kernelArgs});`
      );
    ...
    const getTensorParams = (name: string, dtype: string = "float32") => {
      ...
      return [
        `${dataType}* ${name}_data`,
        `const int* ${name}_shape`,
        `int ${name}_dims`,
      ];
    };

    const inputParams = Array.from(graphInputs.entries()).flatMap(
      ([name, { node, port }]) => {
        const inputSpec = node.inputs.get(port)!;
        return getTensorParams(name, inputSpec.dtype);
      }
    );
    const outputParams = Array.from(graphOutputs.keys()).flatMap((name) =>
      getTensorParams(name, "float32")
    );
    const parameterParams = paramNames.flatMap((name) =>
      getTensorParams(name, "float32")
    );
    const hostFunctionSignatureParams = [
      ...inputParams,
      ...outputParams,
      ...parameterParams,
      "char* workspace",
...
`;
    ...
    for (const [inputName, { node, port }] of graphInputs.entries()) {
      ...
      tensorInstantiations.push(
        `  Tensor<${tensorType}> ${inputName} = {(${dataType}*)${inputName}_data, ${inputName}_shape, ${inputName}_dims};`
      );
    }
    ...
    for (const outputName of graphOutputs.keys()) {
      tensorInstantiations.push(
        `  Tensor<float> ${outputName} = {${outputName}_data, ${outputName}_shape, ${outputName}_dims};`
      );
    }
    ...
    for (const paramName of paramNames) {
      tensorInstantiations.push(
        `  Tensor<float> ${paramName} = {${paramName}_data, ${paramName}_shape, ${paramName}_dims};`
      );
    }
...
${executionCalls.join("\n")}
 ...
 // --- End Execution Flow ---
  ...
  private planMemory(
    ...
    // Third pass: detect in-place operation opportunities (disabled for safety)
