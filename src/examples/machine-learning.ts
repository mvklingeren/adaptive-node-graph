import { Graph, AdaptiveNode, createProcessor, createParallelNode } from './universal-adaptive-router';
...
const featureExtractor = createProcessor<Float32Array, Float32Array>((data) => {
  ...
  features[4] = data.reduce((sum, val, i) => 
    i > 0 ? sum + Math.abs(val - data[i - 1]) : sum, 0
  );
...
// Mock ML model
const model = createProcessor<Float32Array, { class: string; confidence: number }>((features) => {
  ...
  if (sum > 10) {
    ...
    return { class: 'negative', confidence: 0.90 };
  } else {
